...
const {luntan} = require('../../models/config/default')
const user = require('./user')
//建立数据库连接池
const pool = mysql.createPool({
    host: luntan.database.HOST,
    user: luntan.database.USERNAME,
    password: luntan.database.PASSWORD,
    database: luntan.database.DATABASE
});
...
let query = function(sql, values) {
    return new Promise((resolve, reject)=>{
        pool.getConnection(function (err,connection) {
         ...
         console.log('luntan mysql')
         
    })
    })
}
// 两个数据关联     FOREIGN KEY (uid) REFERENCES user.users(id)
...
var posts = `create table if not exists posts(
    id INT(200) NOT NULL AUTO_INCREMENT,
        ...
        PRIMARY KEY (id) ,
        FOREIGN KEY (uid) REFERENCES user.users(id)
        ON DELETE CASCADE
